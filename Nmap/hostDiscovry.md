# 🚀 Host Discovry with Nmap

# nmap -sn known as ping scan [no port scan , no service scan just host discovery]

# What -sn actually does depends on context:

# As root user: [uses all these together]
...
If the target is on the same local subnet [on same network], nmap skips all 4 of these entirely and just uses ARP. [no firewall ]
